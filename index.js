...
app.get('/list', function (req, res) {
    ...
    oracledb.getConnection(connAttrs, function (err, connection) {
        ...
        if (token == '') {
            req.status(404).send(JSON.stringify({
                status: 500,
                message: "Missing Token Mismacth",
                detailed_message: "id token tidak di temukan"
            }));
            ...
            if (err) {
                ...
                res.status(500).send(JSON.stringify({
                    status: 500,
                    message: "Error connecting to DB" + req.query.token,
                    detailed_message: err.message
                }));
                return;
            }

            connection.execute("SELECT * FROM DHN where rownum  <= 10", {}, {
                ...
                if (err) {
                    ...
                    res.status(500).send(JSON.stringify({
                        status: 500,
                        message: "Error getting the user profile" + req.query.id,
                        detailed_message: err.message
                    }));
                } else {
